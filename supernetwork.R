...
rm(list=ls(env=.GlobalEnv))
  ...
  #Defining list of packages required to run this script
packages <- c("dplyr","httr","stringr","gprofiler2","rvest","tidyr","curl")
  #Checking whether required packages are installed, and installing if they are not
installed_packages <- packages %in% rownames(installed.packages())
if (any(installed_packages == FALSE)) {
  install.packages(packages[!installed_packages])
}
  #Slightly different install structure for RCy3 and BridgeDbR since these are installed via BiocManager package
  #Install status of BiocManager is also checked
...
invisible(lapply(c(packages,"RCy3","BridgeDbR"), require, character.only = TRUE))
  ...
  #Getting system time and date for execution time assessment
sysdatetime <- Sys.time()
datetime <- format(sysdatetime, format = "%Y-%m-%d_%Hh%M")
  #Creating directories for outputs generated by this script
  #A new 'session' directory is generated each time the script is ran past this point
  #This directory contains session metadata, networks, tables, and visualisations
  #Such a structure enables improved reproduciblity and enables comparison of outputs derived from dynamic online sources
dir.create("Outputs")
...
dir.create(sprintf("Outputs/Session-%s/Other",datetime))
  #Setting a variable for the directory that networks generated in the current session should be saved to for future reference
nw_savepath <- sprintf("%1$s/Outputs/Session-%2$s/Networks/",getwd(),datetime)
  #Setting a variable for the directory that other files generated in the current session should be saved to for future reference
...
file.create(sprintf("Outputs/Session-%s/metadata.txt",datetime))
  #Defining a function to easily write information to the metadata file of the current session
metadata.add <- function(info) {
  write(sapply(info, as.character), sprintf("Outputs/Session-%s/metadata.txt",datetime),append=TRUE, sep = "\n")
}
  #Adding the current timezone, date, and time to the metadata file
metadata.add(sysdatetime)
metadata.add(Sys.timezone())
metadata.add("")

  #Generating a sessioninfo file for the current session
...
writeLines(capture.output(sessionInfo()),sprintf("Outputs/Session-%s/sessioninfo-%s.txt",datetime,datetime))
  ...
  #Making empty list to store execution times in for script runtime benchmarking
execution_times <- list()

  #Defining a function to record timestamp when the function is called at a given section of the script/workflow
start_section <- function(section_name) {
  execution_times[[section_name]] <<- Sys.time()
}
  #Defining a function to record the timestamp when the function is called at the end of a given section
  #Also calculates difference between start and end time to assess runtime of specific section
end_section <- function(section_name) {
  ...
  #Checking if Cytoscape is running and retrieving version info
cytoscapePing()
cytoscapeVersionInfo()
  #Adding version info to metadata for reproducibility
metadata.add(capture.output(cytoscapeVersionInfo()))

  #Defining function to check if required Cytoscape apps are installed
checkinstall.app <- function(app) {
    #Calling app installation status from Cytscape
  status_string <- getAppStatus(app)
    #Cytoscape returns a string, not a boolean, for app install status
    #Therefore, we check the last word of that string ('installed' | 'uninstalled')
  words <- strsplit(status_string, " ")[[1]]
  last_word <- tail(words, 1)
    #If the last element of the string is 'installed', print that the app is already installed and do nothing
  if (last_word == "Installed") {
    print(sprintf("App %s is already installed.",app))
    } else {
        #If the app is not already installed, install it using the corresponding RCy3 function
    installApp(app)
    print(sprintf("Installed app %s.",app))
    }
}
  #List of required Cytoscape apps
applist <- c("stringApp","clusterMaker2","yFiles Layout Algorithms","CyTargetLinker")
  ...
  #Checking whether required apps are installed
lapply(applist,checkinstall.app)
  #Getting the list of installed Cytoscape apps with their versions and writing to metadata
...
metadata.add("")
  ...
  #Creating a directory that houses BridgeDb bridge files required for identifier mapping
dir.create(paste0(getwd(),"/BridgeDb"))
  #Defining save path for the bridge mapping file
bridgedb_dir <- paste0(getwd(),"/BridgeDb/Hs_Derby_Ensembl_108.bridge")
  #Downloading bridge file for Homo sapiens identifiers to the repo directory
  #File is included in .gitignore to avoid attemtping upload of such a large file to GitHub or other platforms
  #File is quite large (800MB)
getBridgeDbmap <- function(dir = bridgedb_dir, confirmation = "BridgeDb mapping file not detected. Download BridgeDb mapping file for Homo sapiens (800.4 MB)? (yes/no): ") {
    #Checking if the bridge file already exists in the above defined directory 
  if(file.exists(dir)) {
     message("File already present at ", dir, " No files downloaded.")
   } else {
      #Asking user if they want to download the file if it doesn't exist yet
      #Bridge file is needed since identifier mapping is crucial for data harmonisation in this script
...
sparqlquery <- function(endpoint,queryfile,output) {
  ...
  base_url <- "https://sparql.wikipathways.org/sparql/?default-graph-uri=&query="
    ...
    #Reading the text file containing the SPARQL query locally stored in the repo
  querybody <- paste(readLines(file_path), collapse = "")
    #Encoding the string to URL so it can be sent to the endpoint
  encoded_query <- URLencode(querybody)
    #Joining the encoded query with the base URL of the respective targeted endpoint and requesting an HTML output
  full_url <- paste0(base_url, encoded_query, "&format=text/html&timeout=0&signal_void=on")
    #Sending the query to the targeted endpoint and extracting as HTML
  html <- read_html(full_url)
    #DisGeNET has slightly different HTML structure, so somewhat different navigation to output table
  if(tolower(endpoint) %in% c("DisGeNET","DGN","disgenet","Disgenet")) {
    tablebody <- html %>% 
      html_element("body") %>%
      html_element("table") } 
    #Navigating to the output table
  else
  tablebody <- html %>% 
    html_element("body") %>%
    html_element("div") %>%
    html_element("table")
    #Assigning output table to object for further use
  assign(output, html_table(tablebody), envir=.GlobalEnv)
}

  #Defining function to add the data source for imported nodes as an attribute to the node table
createNodeSource <- function(source,doi=NULL) {
    #Generating a network from DisGeNET-derived data results in a different node table syntax
    #Note the 2 spaces between 'default' and 'node'
    #If this check is not done, Cytoscape cannot target the correct node table and will not be able to add node source to it
    #I'm not sure why this is, appears to be some bug in Cytoscape
  if (source == "fromDisGeNET") {
  networkname <- getNetworkName()
  nodetable <- paste0(networkname," default  node") 
  } 
    #Usually, node tables are designated with 'Network name default node', with one space between 'default' and 'node'
  else {
    networkname <- getNetworkName()
    nodetable <- paste0(networkname," default node") 
  }
    #Creating new node attributes for each included source
  ...
  commandsRun(sprintf("table create column columnName=fromSTRING table=%s type=string",nodetable))
    #Different syntax for STRING nodes as these are added via the STRING app instead of through external means
    #Nodes that are currently selected are given the fromSTRING = 1 attribute 
    #Mostly deprecated anyways since no nodes are added from STRING in the current setup of the script,
    #but kept in case this function is needed in the future
  if ( source == "STRINGnode") {
    commandsRun(sprintf('table set values columnName=%1$s handleEquations=false rowList="selected:true" table=%2$s value=1',source,nodetable))
      ...
      #Setting values to 1 in the column describing from which source the node has been imported
      #for the source specific in the argument of this function
  commandsRun(sprintf("table set values columnName=%1$s handleEquations=false rowList=all table=%2$s value=1",source,nodetable))
  }
  if (!is.null(doi)) {
      #Adding publication doi to a dedicated column
      #Allows easy traceback from which publication a node was added if desired
